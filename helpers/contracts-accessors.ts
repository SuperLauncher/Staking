...
export const deployStakedAave = async (
  ...
  const instance = await deployContract<SvLaunch>(id, args);
  if (verify) {
...
export const getMintableErc20 = getContractFactory<MintableErc20>(
...
export const getStakedAaveProxy = async (address?: tEthereumAddress) => {
  return await getContract<InitializableAdminUpgradeabilityProxy>(
    ...
    address ||
      (
        await getDb()
          .get(`${eContractid.StakedAave}.${DRE.network.name}`)
          .value()
      ).address
  );
};
...
export const getStakedAaveImpl = async (address?: tEthereumAddress) => {
  return await getContract<SvLaunch>(
    eContractid.StakedAave,
    address ||
      (
        await getDb()
          .get(`${eContractid.StakedAaveImpl}.${DRE.network.name}`)
          .value()
      ).address
  );
};
...
export const getStakedTokenV2 = async (address?: tEthereumAddress) => {
  return await getContract<StakedTokenV2>(
    ...
    address ||
      (
        await getDb()
          .get(`${eContractid.StakedTokenV2}.${DRE.network.name}`)
          .value()
      ).address
  );
};
...
export const getIErc20Detailed = getContractFactory<Ierc20Detailed>(
  eContractid.IERC20Detailed
);

export const getATokenMock = getContractFactory<ATokenMock>(
  eContractid.ATokenMock
);
...
export const getCRPContract = (address: tEthereumAddress) =>
  getContract<IConfigurableRightsPool>(
    eContractid.IConfigurableRightsPool,
    address
  );
