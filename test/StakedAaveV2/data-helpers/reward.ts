...
export const compareRewardsAtAction = async (
  ...
  if (expectedAccruedRewards.gt("0")) {
    ...
    const eventAccrued = latestReceipt.events?.find(
      ({ event }) => event === "RewardsAccrued"
    );
    if (eventAccrued) {
...
export const compareRewardsAtTransfer = async (
  ...
  const actions = () => [
    stakedAaveV2.connect(from.signer).transfer(toAddress, amount),
  ];
  ...
  expect(toRewardsBalanceAfter).to.bignumber.eq(
  ...
  if (fromAddress === toAddress) {
    ...
    const fromNewBalance = await (
      await stakedAaveV2.balanceOf(fromAddress)
    ).toString();
    const toNewBalance = await (
      await stakedAaveV2.balanceOf(toAddress)
    ).toString();
