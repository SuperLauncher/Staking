...
export const compareRewardsAtAction = async (
  ...
  expect(rewardsBalanceAfter).to.bignumber.eq(
...
export const compareRewardsAtTransfer = async (
  ...
  const actions = () => [
    stakedAave.connect(from.signer).transfer(toAddress, amount),
  ];
  ...
  if (fromAddress === toAddress) {
    ...
    const fromNewBalance = await (
      await stakedAave.balanceOf(fromAddress)
    ).toString();
    const toNewBalance = await (
      await stakedAave.balanceOf(toAddress)
    ).toString();
